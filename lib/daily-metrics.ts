 ...
 * 获取市场指标数据
 */
async function fetchMarketIndicators(): Promise<MarketIndicator[]> {
  // 并行请求所有市场指标 API
  const [
    fngResponse,
    dominanceResponse,
...
/**
 * 获取完整的日报数据（加密货币价格 + 市场指标）
 */
export async function getDailyMetrics(): Promise<DailyMetricsData> {
  const [cryptoPrices, marketIndicators] = await Promise.all([
    fetchCryptoPrices(),
